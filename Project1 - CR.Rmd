...
```{r}
table1(~ gender + ethnicity + school1 + math1 | star1, data = STAR)
```
...
There are 11598 rows and 47 columns in the STAR dataset. Each row represents a student that participated in the experimental phase for at least one year. Each column represents information including demographic variables, school type, class type, test scores, etc.
...
Since there is no violation for ANOVA model assumptions, F-test was used to test whether there is a difference in the math scaled score in 1st grade across students in different type. The hypothesis was shown as below.
