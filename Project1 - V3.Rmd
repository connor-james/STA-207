...
pieGender = plot_ly(count, labels = ~Var1, values = ~Freq, type = 'pie') %>%
  layout(title = "Gender of Participating Students")
...
box1 = plot_ly(star, y = ~math1, color = ~star1, type = "box") %>%
  layout(yaxis = list(title = "First Grade Scaled Math Score"),
...
hist2 <- plot_ly(alpha = 0.1) %>%
  add_histogram(x = ~a$math1, marker = list(color = "#C7E9FF"), name = "Small") %>%
  add_histogram(x = ~b$math1, marker = list(color = "#FFE6C1"), name = "Regular + Aide") %>%
  add_histogram(x = ~c$math1, marker = list(color = "#E8FFD6"), name = "Regular") %>%
...
#### GitHub repo: https://github.com/connor-james/STA-207.git
...
For our analysis, we will consider the following questions.

1. Is there a significant difference in the mean math scores in the 1st grade across different class types?
2. Is the ANOVA model a good fit for the purpose of this analysis? In other words, are the assumptions of the ANOVA model satisfied? To explore this question, we will use model diagnostics such as residual plots and a Levene test for equality of variances.
3. Can we conclude that class sizes affect the scaled math scores of first grade students?
...
There are 11,598 rows and 47 columns in the STAR data set. Each row represents a student that participated in the experimental phase for at least one year. Each column represents information including demographic variables. These variables include school type, class type, and scores from different subject tests. For our analysis, we will primarily focus on the type of class and the scaled math score of first graders. 
...
According to the box plot, students in suburban and rural schools performed better on the total math scaled score in 1st grade. Besides, the bar plot shows that in all types of schools, students in small classes performed slightly better than students in regular classes or regular classes with a full-time teacher aide. However, the effect of school type is not very obvious in this plot, and further analysis might be needed.
...
Considering that some schools might drop out of the project, or the class type might be changed if schools gain or lose students, here we only select students that had full records of math scaled scores, and remained in the same type of class from kindergarten to the 3rd grade.
...
### One-Way ANOVA Model
For our analysis, we will use a variation of the factor effects model to capture the effect of the different class treatments. This model is most useful in our analysis because it represents the expected change in math score compared to a student enrolled in the standard regular class treatment. As shown above, the study effectively randomized many of the different demographic features of participants. This randomization allows us to use this simplified model to generalize the impact of class sizes on test scores across all demographics. Moreover, since our primary question examines the effect of class size on math scores, defining the model in this way allows us to examine the effect of each treatment compared to the regular class treatment. 

The final model selected in this analysis is described below.

$$MathScore = \mu +T_2X_2 + T_3X_3 + \epsilon_{i,j} $$

where:

$X_2 = 1$ if the student is assigned to a small class and $0$ otherwise.   
$X_3 = 1$ if the student is assigned to a regular + aide class and $0$ otherwise.    

In this model, $\mu$ represents the average first-grade math test-score for students enrolled in regular classes. Furthermore, $T_2$ represents the average change in score for students enrolled in small classes and $T_3$ represents the average change in score for students enrolled in regular+aide classes, both compared to the regular class treatment. 

To facilitate pair-wise analysis, we used $\mu_i$ to denote the group mean for each class type, where:     
$\mu_i = \mu$ for $i = 1$;    
$\mu_i = \mu + T_iX_i$ for $i = 2, 3$

Before fitting the model described above, it is important to examine the model assumptions.
...
After fitting the factor effect model described above, we found significant evidence to suggest that at least one of the classroom treatments exhibited a different score distribution compared to that of regular class size.

```{r, echo=FALSE}
kable(anova1, caption  = "ANOVA Table") %>% kable_styling(bootstrap_options = "striped", full_width = F)
```

Both the Regular+Aide & Small class sizes returned an increase in average math scores compared to the regular class treatment group. The average score for students in regular classes was 525.28. Students enrolled in Regular + Aide classes performed on average 4.35 scaled points higher than regular class students. Similarly, Students enrolled in small classes performed on average 13.40 scaled points higher than regular class students.


```{r, echo = FALSE, warning=FALSE}
effect = (data.frame(reg1$coefficients))
effect1 = data.frame( "Class Size" = c("Regular", "Small (T2)", "Regular + Aide (T3)"), "Average Math Score" = c(525.28,538.68, 529.63), "Change from Regular" = c("",13.40, 4.35), "Significant" = c("","YES", "YES"))

div(box1 %>% 
      layout(autosize = F, width = 800, height = 400),
    align = "right")

kable(effect1, caption = "Effect of Each Class Size") %>% kable_styling(bootstrap_options = "striped", full_width = F)
...
From the residual normal Q-Q plot below, we can observe that our specified model follows the expected Q-Q quite well. While there are some minor deviations at the tails, which we observed earlier in the the Histogram, the overall trend of the data demonstrates a largely normal distribution. Therefore, we can satisfy the normality assumption. 
...
From the Residual vs Fitted plot below, it is clear that our data is independent and randomly distributed about zero. Therefore, assumption of independence was satisfied. 
...
Finally, the histogram of residuals below show that their distribution in each of the three treatments is indistinguishable. Paired with our earlier test for the equality of variance, we can satisfy this assumption.
...
Since there is no violation for ANOVA model assumptions, an F-test was used to test whether there is a difference in the math scaled score in 1st grade across students in different types of classes. The null and alternative hypotheses were shown as below.

$$H_0: \mu1 = \mu2 = \mu3 \ V.S \ H_a: not\  all\  \mu_i's\  are\ equal$$

Since $F^* = \frac{MSTR}{MSE} = 53.26 > F(1-\alpha; r, n_T-r) = 2.997093$, we reject the null hypothesis at level of 0.05. Not all class types have the same math scaled score.   

To further investigate the difference among the class types, multiple comparisons were used. The hypotheses were shown as below.

$$H_0: D_{ij} = \mu_i-\mu_j, 1\leq i < j \leq3$$

The multipliers of Bonferroni's procedure, Tukey's procedure, and Scheffé's procedure were shown in the table below. It is shown that the Scheffé's multiplier is the smallest. So the Scheffé's procedure was used to construct the confidence intervals. 
...
Our ANOVA model and confidence intervals all demonstrated significant evidence that the scaled math scores for first grade students in small and regular + aide classes were both significantly higher than the scaled math scored for students enrolled in regular classes. Scaled math score of small classes was also significantly higher than that of regular + aide classes in 1st grade.   

***
# Conclusions

Through our analysis, we found significant evidence which demonstrated that on average, students enrolled in both small & regular + aide classes outperformed their peers enrolled in regular sized classes in first-grade math. Because of the high sample size and the rigorously randomized nature of the study, we may make a limited causal inference into the effect of class sizes on a student's mathematical performance. With over 6,000 evaluations of first grade students, we feel confident that we have observed the potential average outcomes of each of the three treatments. Because of this, we can conclude that, the student to instructor ratios in first grade math classes have a causal relationship with the average performance on a standardized math test.

While we can not make any specific conclusions about the causal effect of class size with an individual student's performance, our average causal conclusion is of great significance for policy makers and educators. We have demonstrated that for Tennessee students, enrolling in smaller class sizes or in regular-size classes with an aide in the first grade help them perform better, on average, on the standardized math test. With this conclusion, schools should attempt to provide their students with the best student to teacher ratio possible to provide them the best opportunity to learn and perform on standardized math tests.  

***

# Reference

https://www.wqqaft.org/sites/default/files/periodicals/STARSummer99.pdf
